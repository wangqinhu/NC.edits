...
my $dnds_file = "data/dnds.txt";
my $sam_dir = "data/sam.red";
my $dnds_bin = "dnds_bin";
my $out_dir = "dnds_vs_fnfs";

system("mkdir -p $out_dir");

print STDERR "Processing data/NC.edit.cds.v20161024.txt ...\n";
dnds_red($edit_file, "$out_dir/red.txt");

opendir (DIR, "$sam_dir") or die "Cannot open $sam_dir: $!\n";
foreach my $file (sort readdir DIR) {
	next unless $file =~ /.txt$/;
	print STDERR "Processing $sam_dir/$file ...\n";
	dnds_red("$sam_dir/$file", "$out_dir/$file");
}
closedir DIR;
...
sub dnds_red {
	...
	# load data
	my $dnds = load_dnds($dnds_file);
	my $edit = load_edit($edit_file);

	# dnds group ids
	my @grp_id = ();
	my $num_of_genes = keys $dnds;
	my $sub_num = int($num_of_genes / $num_of_bin + 0.5) ;
	my ($i, $j, $k) = (0, 0, 0);
	for my $gene_id (sort {$dnds->{$a} cmp $dnds->{$b}} keys $dnds) {
		$i = int ($k / $sub_num);
	...
	open (TAB, ">$fnfs_file") or die $!;
	# print header
	print TAB "grp\tN\tS\tn\ts\tdnds\tfn\tfs\n";

	system("mkdir -p $dnds_bin");
	...
	for my $i (0..$#grp_id) {
		...
		# calculate N and S
		open (OUT, ">$dnds_bin/grp.$i.id.txt") or die $!;
		print OUT $genes[$i];
		close OUT;
		system("perl ./bin/id2seq.pl $dnds_bin/grp.$i.id.txt $cds_seq_file $dnds_bin/grp.$i.cds.fa");
		my $ns = `perl ./bin/a2g_ns_6.pl $dnds_bin/grp.$i.cds.fa`;
		($N[$i], $S[$i]) = split /\t/, $ns;

		# calculate mean dnds
		my @dnds_vals = split /\n/, $dnds[$i];
		my $median_dnds = median(@dnds_vals);
		...
		my $line = join("\t", $i, $N[$i], $S[$i], $n[$i], $s[$i], $median_dnds, $fn*1000, $fs*1000);
		$line =~ s/\t$//;
		print TAB $line, "\n";
	}

	system("rm -rf $dnds_bin");
	close TAB;
